						...
						const cmsStride* Stride)
{
    ...
    slices  = (_cmsWorkSlice*)_cmsCalloc(nSlices, sizeof(_cmsWorkSlice));
    handles = (cmsHANDLE*) _cmsCalloc(nSlices, sizeof(cmsHANDLE));

    if (slices == NULL || handles == NULL)
    {
        if (slices)  _cmsFree(slices);
        if (handles) _cmsFree(handles);
    ...
    if (_cmsThrSplitWork(&master, nSlices, slices))
    {
        ...
        for (i = 1; i < nSlices; i++)
        {
            handles[i] = _cmsThrCreateWorker(worker, &slices[i]);
        }
